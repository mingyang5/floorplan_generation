...
class WallEncoderLayer(nn.Module):
    def __init__(self, d_model, heads, dropout, activation):
        ...
        self.ff = FeedForward(d_model, d_model*2, dropout, activation)      # Linear + Dropout + Linear
...
class WallEncoderLayer_v1(nn.Module):
    def __init__(self, d_model, heads, dropout, activation):
        super().__init__()
        self.norm_1 = nn.InstanceNorm1d(d_model)
        self.norm_2 = nn.InstanceNorm1d(d_model)
...
class EncoderLayerWithStructuralCrossAttention(nn.Module):
    ...
    def forward(self, x, self_mask, gen_mask, x_structural, structural_mask, door_only_mask, passage_ony_mask, entrance_only_mask, door_type_emb):
        ...
        x2 = self.norm_1(x)                                         # torch.Size([48, 241, 512])
        x2_structural = self.norm_struct(x_structural)              # torch.Size([48, 57, 512])
        door_type_emb = self.norm_door_type(door_type_emb)          # torch.Size([3, 512])

        x = x \
                + self.dropout(self.self_attn(x2, x2, x2, self_mask)) \
                + self.dropout(self.gen_attn(x2, x2, x2, gen_mask)) # torch.Size([48, 241, 512])
        
        x_door_only_key_value = x2 + door_type_emb[0, :]            # torch.Size([48, 241, 512])
        x_passage_only_key_value = x2 + door_type_emb[1, :]
        x_entrance_only_key_value = x2 + door_type_emb[2, :]

        x += self.dropout(self.door_attn(x2, x_door_only_key_value, x_door_only_key_value, door_only_mask))                 # torch.Size([48, 241, 512])
...
class EncoderLayerWithStructuralCrossAttention_v1(nn.Module):
    def __init__(self, d_model, heads, dropout, activation):
        super().__init__()
        self.norm_1 = nn.InstanceNorm1d(d_model)
        self.norm_2 = nn.InstanceNorm1d(d_model)
    ...
    def forward(self, x, self_mask, gen_mask, x_structural, structural_mask, door_only_mask, passage_ony_mask, entrance_only_mask, door_type_emb):
        ...
        # x = x + self.dropout(self.self_attn(x2, x2, x2, self_mask)) + self.dropout(self.gen_attn(x2, x2, x2, gen_mask))
        ...
        x = x + self.dropout(self.struct_attn(q=x2, k=x2_structural, v=x2_structural, mask=structural_mask))
...
class EncoderLayerWithStructuralCrossAttention_v2(nn.Module):
    def __init__(self, d_model, heads, dropout, activation):
        super().__init__()
        self.norm_1 = nn.InstanceNorm1d(d_model)
        self.norm_2 = nn.InstanceNorm1d(d_model)
    ...
    def forward(self, x, self_mask, gen_mask, x_structural, structural_mask, door_only_mask, passage_ony_mask, entrance_only_mask, door_type_emb):
        ...
        x = x + self.self_attn(x2, x2, x2, self_mask) 
        x = x + self.gen_attn(x2, x2, x2, gen_mask)
        x = x + self.struct_attn(q=x2, k=x2_structural, v=x2_structural, mask=structural_mask)
...
class TransformerModel(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.in_channels = in_channels                      # 18
        self.condition_channels = condition_channels        # 115
        self.model_channels = model_channels                # 512
        self.out_channels = out_channels                    # 2
        self.time_channels = model_channels                 # 512
        self.use_checkpoint = use_checkpoint                # False
        self.analog_bit = analog_bit                        # False
        self.use_unet = use_unet                            # False
        self.num_transfomers_layers = 12
        

        self.struct_in_channels = struct_in_channels        # 18
        ...
        self.time_embed = nn.Sequential(
            nn.Linear(self.model_channels, self.model_channels),            # input_channel = output_channel
            nn.SiLU(),
            nn.Linear(self.model_channels, self.time_channels),
        )
        # self.input_emb = nn.Linear(self.in_channels, self.model_channels)               # 18, 512 -> 是不是可以加一个位置嵌入
        # self.condition_emb = nn.Linear(self.condition_channels, self.model_channels)
        self.input_emb = nn.Sequential(
            nn.Linear(self.in_channels, self.model_channels),
            nn.SiLU(),
            nn.Linear(self.model_channels, self.model_channels),
        )
        self.condition_emb = nn.Sequential(
            nn.Linear(self.condition_channels, self.model_channels),
            nn.SiLU(),
            nn.Linear(self.model_channels, self.model_channels),
        )
        self.conditon_for_wall_emb = nn.Sequential(
            # nn.Linear(self.corners, self.struct_in_channels),
        ...
        if self.use_structural_cross_attention:
            self.struct_in_channels: int = self.struct_in_channels # type: ignore, 18
            ...
            self.transformer_layers: typing.List[EncoderLayerWithStructuralCrossAttention_v1] = nn.ModuleList([EncoderLayerWithStructuralCrossAttention_v1(self.model_channels, 4, 0.1, self.activation) 
                                                                                                            for x in range(self.num_transfomers_layers)]) # type: ignore 
            self.struct_emb = nn.Linear(self.struct_in_channels, self.model_channels)

            # if self.use_wall_self_attention: # True
            #     self.wall_self_attention_layers = nn.ModuleList([WallEncoderLayer(self.model_channels, 4, 0.1, self.activation) 
            #                                                      for x in range(self.num_transfomers_layers)]) # type: ignore
                
            if self.use_wall_self_attention: # True
                self.wall_self_attention_layers = nn.ModuleList([WallEncoderLayer_v1(self.model_channels, 4, 0.1, self.activation) 
                                                                 for x in range(self.num_transfomers_layers)]) # type: ignore
        ...
        if not self.analog_bit:
            ...
            if self.use_structural_cross_attention:
                # self.output_linear_bin2 = EncoderLayerWithStructuralCrossAttention(self.model_channels, heads=1, dropout=0.1, activation=self.activation)
                # self.output_linear_bin3 = EncoderLayerWithStructuralCrossAttention(self.model_channels, heads=1, dropout=0.1, activation=self.activation)
                self.output_linear_bin2 = EncoderLayerWithStructuralCrossAttention_v1(self.model_channels, heads=1, dropout=0.1, activation=self.activation)
                self.output_linear_bin3 = EncoderLayerWithStructuralCrossAttention_v1(self.model_channels, heads=1, dropout=0.1, activation=self.activation)

                self.struct_linear_bin1 = nn.Linear(2 * 9 + 2 * 9 * self.num_bits + self.model_channels, self.model_channels)       # 692
            else:
                self.output_linear_bin2 = EncoderLayer(self.model_channels, heads=1, dropout=0.1, activation=self.activation)
                self.output_linear_bin3 = EncoderLayer(self.model_channels, heads=1, dropout=0.1, activation=self.activation)
    ...
    def forward(self, x, timesteps, xtalpha, epsalpha, is_syn=False, x_structural: th.Tensor=None, **kwargs) -> typing.Tuple[th.Tensor, th.Tensor]:
        ...
        time_emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))       # torch.Size([48, 512])    
        time_emb = time_emb.unsqueeze(1)
        
        # Embedding of the corners of shape: N x corners x feats (feats==self.model_channels)
        input_emb = self.input_emb(x)                               # torch.Size([48, 273, 18]

        if self.condition_channels <= 1:                            # self.condition_channels=115
        ...
        for key in [f'{prefix}room_types', f'{prefix}corner_indices', f'{prefix}room_indices']:
            if cond is None:
                ...
                cond = th.cat((cond, kwargs[key]), 2)       # torch.Size([48, 289, 15]), torch.Size([48, 289, 4]), torch.Size([48, 289, 96]) -> concat 一些数据
        ...
        cond_for_wall_emb = self.condition_emb(cond.float())
        cond_for_wall_emb = cond_for_wall_emb.permute(0, 2, 1)
        cond_for_wall_emb = self.conditon_for_wall_emb(cond_for_wall_emb)
        cond_for_wall_emb = cond_for_wall_emb.permute(0, 2, 1)
        ...
        if self.use_structural_cross_attention:
            ...
            cond_for_wall_emb = cond_for_wall_emb.repeat((1, struct_emb.shape[1], 1))
            if self.use_wall_self_attention:
                wall_out = struct_emb + cond_for_wall_emb
